...
def train(model, data_loader, train_optimizer, epoch, epochs, batch_size=32, temperature=0.5, device='cuda'):
    ...
    for data_pair in train_bar:
        ...
        # pass augmented images through the model
        # model.forward() returns (feature, out) where out is the projection head output
        _, out_left = model(x_i)
        _, out_right = model(x_j)

        # compute contrastive loss using the projection head outputs
