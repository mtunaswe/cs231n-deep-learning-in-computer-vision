...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # compute the activation: next_h = tanh(x @ Wx + prev_h @ Wh + b)
...
def rnn_step_backward(dnext_h, cache):
    ...
    # gradient through tanh: dtanh = (1 - tanh^2(a))
    # since next_h = tanh(a), we have dtanh = 1 - next_h^2
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        # add gradient from current timestep to gradient flowing from future
...
def word_embedding_backward(dout, cache):
    ...
    x, W = cache             # unpack cache
    dW = np.zeros_like(W)    # initialize gradient for embedding matrix

    # accumulate gradients for each word index
    # np.add.at handles the case where same word appears multiple times
    np.add.at(dW, x, dout)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # compute activation vector: a = x @ Wx + prev_h @ Wh + b
    # and split to four parts, input, forget, output, block input gates
    a = np.hsplit(x @ Wx + prev_h @ Wh + b, 4)

    # apply activation functions to compute gates
    i = sigmoid(a[0])
    f = sigmoid(a[1])
    o = sigmoid(a[2])
    g = np.tanh(a[3])

    next_c = f * prev_c + i * g      # compute next cell state: c_t = f * c_{t-1} + i * g
    next_h = o * np.tanh(next_c)     # compute next hidden state: h_t = o * tanh(c_t)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # backprop into h_t = o * tanh(c_t)
    # dnext_h is (N, H)
    dtanh_next_c = dnext_h * o
    do = dnext_h * np.tanh(next_c)

    # backprop through tanh: derivative of tanh(x) = 1 - tanh^2(x)
    dnext_c_from_h = dtanh_next_c * (1 - np.tanh(next_c) ** 2)

    # add gradient from upstream dnext_c
    dnext_c_total = dnext_c + dnext_c_from_h

    # backprop into c_t = f * c_{t-1} + i * g
    ...
    # for sigmoid: dsigmoid(x)/dx = sigmoid(x) * (1 - sigmoid(x))
    da_i = di * i * (1 - i)
    da_f = df * f * (1 - f)
    da_o = do * o * (1 - o)
    # for tanh: dtanh(x)/dx = 1 - tanh^2(x)
    da_g = dg * (1 - g ** 2)

    # concatenate gradients for the activation vector
    # a was split as [a_i, a_f, a_o, a_g]
    da = np.concatenate([da_i, da_f, da_o, da_g], axis=1)

    # backprop through a = x @ Wx + prev_h @ Wh + b
    dx = da.dot(Wx.T)
    dprev_h = da.dot(Wh.T)
    dWx = x.T.dot(da)
    dWh = prev_h.T.dot(da)
...
def lstm_backward(dh, cache):
    ...
    # initialize gradient flowing back through hidden and cell states
    dprev_h = np.zeros((N, H))
    ...
    for t in reversed(range(T)):
        ...
        # backward pass for this timestep
        dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h, dprev_c, cache[t])
