...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # (N, T) -> (N, T, W) where W is wordvec_dim
        caption_embeddings = self.embedding(captions)
        caption_embeddings = self.positional_encoding(caption_embeddings)

        # project image features to same dimension as word vectors
        # (N, D) -> (N, W) -> (N, 1, W)
        ...
        tgt_mask = torch.tril(torch.ones(T, T)).type(torch.bool)

        # apply transformer decoder
        # the decoder attends to both the captions (self-attention with mask)
        # and the image features (cross-attention)
        decoder_output = self.transformer(caption_embeddings, features_projected, tgt_mask=tgt_mask)

        # project decoder output to vocabulary scores
        # (N, T, W) -> (N, T, V)
