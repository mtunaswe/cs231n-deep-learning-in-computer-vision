...
def discriminator_loss(logits_real, logits_fake):
    ...
    # So we minimize the negative (which is the loss):
    # -E[log D(x)] - E[log(1 - D(G(z)))]

    # create labels: real images should have label 1, fake images should have label 0
...
def generator_loss(logits_fake):
    ...
    # create labels: generator wants discriminator to think fake images are real (label 1)
    batch_size = logits_fake.size(0)
    real_labels = torch.ones(batch_size).type(dtype)

    # compute loss - we want discriminator to output high scores for fake images
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    # (1/2) * E[(D(x) - 1)^2] + (1/2) * E[D(G(z))^2]
    # We want D(real) to be close to 1, and D(fake) to be close to 0
...
def ls_generator_loss(scores_fake):
    ...
    # (1/2) * E[(D(G(z)) - 1)^2]
    # Generator wants D(fake) to be close to 1 (fool the discriminator)
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
        ...
        nn.Linear(4 * 4 * 64, 4 * 4 * 64),          # -> (N, 1024)
        nn.LeakyReLU(0.01),
        nn.Linear(4 * 4 * 64, 1)                    # -> (N, 1)
    ) 
